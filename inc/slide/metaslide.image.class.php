...
class MetaImageSlide extends MetaSlide {
    ...
    public function ajax_create_slide() {
        ...
        if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'metaslider_addslide' ) ) {
            echo "<tr><td colspan='2'>" . __( "Security check failed. Refresh page and try again.", "rhswp-rijksvideo" ) . "</td></tr>";
            wp_die();
        }
        ...
        if ( is_array( $selection ) && count( $selection ) && $slider_id > 0 ) {
            ...
            foreach ( $selection as $slide_id ) {
                ...
                if ( $this->slide_exists_in_slideshow( $slider_id, $slide_id ) ) {
                    ...
                    echo "<tr><td colspan='2'>ID: {$slide_id} \"" . get_the_title( $slide_id ) . "\" - " . __( "Failed to add slide. Slide already exists in slideshow.", "rhswp-rijksvideo" ) . "</td></tr>";

                } else if ( ! $this->slide_is_unassigned_or_image_slide( $slider_id, $slide_id ) ) {

                    echo "<tr><td colspan='2'>ID: {$slide_id} \"" . get_the_title( $slide_id ) . "\" - " . __( "Failed to add slide. Slide is not of type 'image'.", "rhswp-rijksvideo" ) . "</td></tr>";

                } else if ( ! wp_attachment_is_image( $slide_id ) ) {

                    echo "<tr><td colspan='2'>ID: {$slide_id} \"" . get_the_title( $slide_id ) . "\" - " . __( "Failed to add slide. Slide is not an image.", "rhswp-rijksvideo" ) . "</td></tr>";
    ...
    public function ajax_resize_slide() {
        ...
        $settings = get_post_meta( $slider_id, 'rijksvideo_settings', true );
    ...
    protected function get_admin_slide() {
        ...
        $slide_label = apply_filters( "metaslider_image_slide_label", __( "Image Slide", "rhswp-rijksvideo" ), $this->slide, $this->settings );
    ...
    public function get_admin_tabs() {
        ...
        $url = esc_attr( get_post_meta( $slide_id, 'rijksvideo_url', true ) );
        $title = esc_attr( get_post_meta( $slide_id, 'rijksvideo_title', true ) );
        $target = get_post_meta( $slide_id, 'rijksvideo_new_window', true ) ? 'checked=checked' : '';
        $caption = esc_textarea( $this->slide->post_excerpt );

        $general_tab = "<textarea name='attachment[{$slide_id}][post_excerpt]' placeholder='" . __( "Caption", "rhswp-rijksvideo" ) . "'>{$caption}</textarea>
                        <input class='url' type='text' name='attachment[{$slide_id}][url]' placeholder='" . __( "URL", "rhswp-rijksvideo" ) . "' value='{$url}' />
                        <div class='new_window'>
                        <label>" . __( "New Window", "rhswp-rijksvideo" ) . "<input type='checkbox' name='attachment[{$slide_id}][new_window]' {$target} /></label>
                        </div>";

        if ( ! $this->is_valid_image() ) {
            $message = __( "Warning: Image data does not exist. Please re-upload the image.", "rhswp-rijksvideo" );

            $general_tab = "<div class='warning'>{$message}</div>" . $general_tab;
        }

        $seo_tab = "<div class='row'><label>" . __( "Image Title Text", "rhswp-rijksvideo" ) . "</label></div>
                    <div class='row'><input type='text' size='50' name='attachment[{$slide_id}][title]' value='{$title}' /></div>
                    <div class='row'><label>" . __( "Image Alt Text", "rhswp-rijksvideo" ) . "</label></div>
                    <div class='row'><input type='text' size='50' name='attachment[{$slide_id}][alt]' value='{$alt}' /></div>";

        $tabs = array(
            'general' => array(
                'title' => __( "General", "rhswp-rijksvideo" ),
                'content' => $general_tab
            ),
            'seo' => array(
                'title' => __( "SEO", "rhswp-rijksvideo" ),
                'content' => $seo_tab
            )
        );
        ...
        if ( version_compare( get_bloginfo('version'), 3.9, '>=' ) ) {
            ...
            $crop_position = get_post_meta( $slide_id, 'rijksvideo_crop_position', true);

            if ( ! $crop_position ) {
                $crop_position = 'center-center';
            }

            $crop_tab = "<div class='row'><label>" . __( "Crop Position", "rhswp-rijksvideo" ) . "</label></div>
                        <div class='row'>
                            <select class='crop_position' name='attachment[{$slide_id}][crop_position]'>
                                <option value='left-top' " . selected( $crop_position, 'left-top', false ) . ">" . __( "Top Left", "rhswp-rijksvideo" ) . "</option>
                                <option value='center-top' " . selected( $crop_position, 'center-top', false ) . ">" . __( "Top Center", "rhswp-rijksvideo" ) . "</option>
                                <option value='right-top' " . selected( $crop_position, 'right-top', false ) . ">" . __( "Top Right", "rhswp-rijksvideo" ) . "</option>
                                <option value='left-center' " . selected( $crop_position, 'left-center', false ) . ">" . __( "Center Left", "rhswp-rijksvideo" ) . "</option>
                                <option value='center-center' " . selected( $crop_position, 'center-center', false ) . ">" . __( "Center Center", "rhswp-rijksvideo" ) . "</option>
                                <option value='right-center' " . selected( $crop_position, 'right-center', false ) . ">" . __( "Center Right", "rhswp-rijksvideo" ) . "</option>
                                <option value='left-bottom' " . selected( $crop_position, 'left-bottom', false ) . ">" . __( "Bottom Left", "rhswp-rijksvideo" ) . "</option>
                                <option value='center-bottom' " . selected( $crop_position, 'center-bottom', false ) . ">" . __( "Bottom Center", "rhswp-rijksvideo" ) . "</option>
                                <option value='right-bottom' " . selected( $crop_position, 'right-bottom', false ) . ">" . __( "Bottom Right", "rhswp-rijksvideo" ) . "</option>
                            </select>
                        </div>";

            $tabs['crop'] = array(
                'title' => __( "Crop", "rhswp-rijksvideo" ),
                'content' => $crop_tab
            );
    ...
    protected function get_public_slide() {
        ...
        $slide = array(
            'id' => $this->slide->ID,
            'url' => __( get_post_meta( $this->slide->ID, 'rijksvideo_url', true ) ),
            'title' => __( get_post_meta( $this->slide->ID, 'rijksvideo_title', true ) ),
            'target' => get_post_meta( $this->slide->ID, 'rijksvideo_new_window', true ) ? '_blank' : '_self',
            'src' => $thumb,
