...
class MetaSlide {
    ...
    public function set_slider( $id ) {
        ...
        $this->settings = get_post_meta( $id, 'rijksvideo_settings', true );
    }
    ...
    public function ajax_change_slide_image() {
        ...
        if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'metaslider_changeslide' ) ) {
            wp_die( json_encode( array(
                    'status' => 'fail',
                    'msg' => __( "Security check failed. Refresh page and try again.", "rhswp-rijksvideo" )
                )
            ));
        }
        ...
        wp_die( json_encode( array(
                'status' => 'fail',
                'msg' => __( "File copy failed. Please check upload directory permissions.", "rhswp-rijksvideo" )
            )
        ));
    }
    ...
    public function slide_exists_in_slideshow( $slider_id, $slide_id ) {
        ...
        return has_term( "{$slider_id}", RIJKSVIDEO_CT, $slide_id );
    ...
    public function slide_is_unassigned_or_image_slide( $slider_id, $slide_id ) {
        ...
        $type = get_post_meta( $slide_id, 'rijksvideo_type', true );
    ...
    public function tag_slide_to_slider() {
        ...
        if ( ! term_exists( $this->slider->ID, RIJKSVIDEO_CT ) ) {
            // create the taxonomy term, the term is the ID of the slider itself
            wp_insert_term( $this->slider->ID, RIJKSVIDEO_CT );
        }

        // get the term thats name is the same as the ID of the slider
        $term = get_term_by( 'name', $this->slider->ID, RIJKSVIDEO_CT );
        // tag this slide to the taxonomy term
        wp_set_post_terms( $this->slide->ID, $term->term_id, RIJKSVIDEO_CT, true );
    ...
    protected function detect_self_metaslider_shortcode( $content ) {
        ...
        if ( preg_match_all( '/'. $pattern .'/s', $content, $matches ) && array_key_exists( 2, $matches ) && ( in_array( 'rijksvideo', $matches[2] ) || in_array( 'rhswp-rijksvideo', $matches[2] ) ) ) {
