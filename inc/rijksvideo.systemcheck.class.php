...
class RijksvideoSystemCheck {
    ...
    public function __construct() {
        $this->options = get_site_option( 'rijksvideo_systemcheck' );
    }
    ...
    private function dismissMessages() {
        if ( isset( $_REQUEST['dismissMessage'] ) && isset( $_REQUEST['_wpnonce'] ) ) {
            ...
            if ( wp_verify_nonce( $nonce, "rijksvideo-dismiss-{$key}" ) ) {
                $this->options[$key] = false;
                update_site_option( 'rijksvideo_systemcheck', $this->options );
            }
        }
    }
    ...
    private function checkWordPressVersion() {
        ...
        if ( !function_exists( 'wp_enqueue_media' ) ) {
            $error = "Rijksvideo requires WordPress 3.5 or above. Please upgrade your WordPress installation.";
    ...
    private function checkImageLibrary() {
        ...
        if ( ( !extension_loaded( 'gd' ) || !function_exists( 'gd_info' ) ) && ( !extension_loaded( 'imagick' ) || !class_exists( 'Imagick' ) || !class_exists( 'ImagickPixel' ) ) ) {
            $error = "Rijksvideo requires the GD or ImageMagick PHP extension. Please contact your hosting provider";
    ...
    private function printMessage( $message, $key ) {
        $nonce = wp_create_nonce( "rijksvideo-dismiss-{$key}" );
